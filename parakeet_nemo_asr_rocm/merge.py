...
def _normalise(text: str) -> str:
    """Lower-case helper for token matching."""
...
def merge_longest_common_subsequence(
    ...
    *,
    ...
    # Map overlapping indices back to original arrays
    ...
    for idx in range(len(lcs_pairs)):
        ...
        merged.append(a[ia])  # the shared token
        ...
        if idx < len(lcs_pairs) - 1:
            ...
            gap_b = b[ib + 1 : next_ib]
            merged.extend(gap_a if len(gap_a) >= len(gap_b) else gap_b)

    merged.extend(b[lcs_indices_b[-1] + 1 :])
    return merged
